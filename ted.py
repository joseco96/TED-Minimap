...
Original file is located at
    https://colab.research.google.com/drive/1Dd0buxVAk9lEtiW5W_lIQ_QZR1Lt-z_z
"""
...
def check_tiles(data,type):
    x=data['x']
    y=data['y']

    for i in range(-1, 2):
        for j in range(-1, 2):
            cond=(config.extra_info['map']['x'] == x+i ) & (config.extra_info['map']['z'] == y+j)
            tile = config.extra_info['map'][cond].key.values[0]
            if tile == type:
                return True
    return False
...
def process_event(data,player,config):
  ...
  rubble = False
  victim= check_tiles(data,'green') or check_tiles(data,'yellow') or check_tiles(data,'red')
  if data['role'] == 'engineer':  
    rubble = check_tiles(data,'rubble')

  

  '''if player_data['last_pos_elapsed_s'] == elapsed_s:
      return'''
  ...
  if data['event'] == 'door' :
    ...
    player_data['dig_rubble_duration_s'] += 0.5
  ...
  elif data['event'] == 'triage in-progress' and rubble :



    if (elapsed_s-player_data['skill_end'])<2:
        return
    record_skill_start(data,'dig_rubble', player_data)
  elif data['event'] == 'triage in-progress' and victim:
    if (elapsed_s-player_data['skill_end'])<2:
        return
...
def compute_skills(data,msg_data, config):
    ...
    for player_data in config.state['players'].values():
        ...
        indv_msg['inaction_stand_s']=player_data['inaction_stand_s']
        msg_data['inaction_stand_s']+=player_data['inaction_stand_s']
        if data['timestamp']==1651762095:
            sf=1651762095
        ...
        indv_msg['action_triage_s']=player_data['triage_duration_s']
        msg_data['action_triage_s']+=player_data['triage_duration_s']
        ...
        player_data['triage_start_time'] = None
        player_data['dig_rubble_start_time'] = None
        ...
        check_dict[player_data['id']+'_triage'] = str(player_data['triage_duration_s']) 
        check_dict[player_data['id']+'_inaction'] = str(player_data['inaction_stand_s']) 
    ...
    # Reset bookkeeping.
    reset_player_field('inaction_stand_s', config)
    for skill in ('triage', 'speedup','dig_rubble', 'move_victim', 'explore','triage_red' ,'triage_yellow', 'triage_green' ):
...
def compute_process_values(msg_data, config):
    ...
    msg_data['triage_workload'] = 0.45*(msg_data['triage_count_red'] / config.extra_info['total_triage_red'])+\
        0.3*(msg_data['triage_count_yellow'] / config.extra_info['total_triage_yellow']) +\
        0.25*(msg_data['triage_count_green'] / config.extra_info['total_triage_green'])
...
def main(message,config,check_file):
    ...
    for player in message.keys():
        ...
        if check:
            ...
            check_dict[player+'_triage'] = str(config.state['players'][player]['triage_duration_s']) 
            check_dict[player+'_inaction'] = str(config.state['players'][player]['inaction_stand_s']) 
...
for name in os.listdir(directory):
    ...
    df = pd.DataFrame.from_records(config.state['msg_data'])
    msg_filename = 'output/'+name+'_global.csv'
    ...
    for player in config.state['players']:
        ...
        filename = 'output/'+name+'_'+player+'.csv'
